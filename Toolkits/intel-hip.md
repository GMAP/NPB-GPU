...
# Sources
- [ChipStar Getting Started Guide](https://github.com/CHIP-SPV/chipStar/blob/main/docs/Getting_Started.md)
...
# [Install ChipStar dependencies](https://github.com/CHIP-SPV/chipStar/blob/main/docs/Getting_Started.md)
...
### Export clang-offload-bundler
```
ls /usr/lib/llvm-17/bin/clang-offload-bundler
export PATH=/usr/lib/llvm-17/bin:$PATH
```

### Install ChipStar
```
cd && git clone https://github.com/CHIP-SPV/chipStar.git && cd chipStar

mkdir -p $HOME/chipStar/install

mkdir build && cd build

cmake .. \
  -DLLVM_CONFIG_BIN=/usr/bin/llvm-config-17 \
  -DCMAKE_INSTALL_PREFIX=$HOME/chipStar/install \
  -DCMAKE_BUILD_TYPE=Release \
  -DCHIP_DEBUG=OFF # use ON for debug

make all build_tests install -j$(nproc)
```

### Post-build verification
```
# Attention: chipStar_tests only runs correctly if the Intel + Level Zero driver is installed and working.
"$HOME/chipStar/install/bin/chipStar_tests"
```

### Configure environment variables
```
mkdir -p ~/env
cat << 'EOF' > ~/env/intel-hip.sh
if [ -f ~/intel/oneapi/setvars.sh ]; then source ~/intel/oneapi/setvars.sh --force; fi
INTEL_CHIPSTAR=~/chipStar
export HIP_PLATFORM=intel
export CHIP_DEVICE_TYPE=GPU
export CHIP_BE=level0
export PATH=${INTEL_CHIPSTAR}/install/bin:$PATH
export LD_LIBRARY_PATH=${INTEL_CHIPSTAR}/install/lib:$LD_LIBRARY_PATH
EOF
```

### Compile and run a HIP program with Intel (level zero)
1. Load intel-hip environment variables
```
source ~/env/intel-hip.sh
hipcc --version
```

2. Environment variables in case you want to disable the debug
```
export CHIP_LOG_LEVEL=OFF
export CHIP_DEBUG=0
export CHIP_TRACE=0
```

3. Create the hello.cpp program:
```c++
...
4. Compile and run the hello.cpp program:
```
